...
def launch_vms(size, cpu, ram):
    logging.info(f"Launching {size} VMs with {cpu} CPUs and {ram}MB RAMeach")
    for i in range(size):
        start_time = time.time() # Record start time
        vm_name = "manager" if i == 0 else f"worker{i}"
        subprocess.run(f"multipass launch -n {vm_name} --cpus {cpu} --memory {ram}M --cloud-init cloud-init.yaml", shell=True)
...
def generate_ssh_keys(size):
    ...
    for i in range(size):
        ...
        subprocess.run(f"multipass exec {vm_name} -- ssh-keygen -q -t rsa -C {vm_name} -f /home/ubuntu/.ssh/id_rsa -N \"\"", shell=True)
        public_key = subprocess.run(f"multipass exec {vm_name} -- cat/home/ubuntu/.ssh/id_rsa.pub",capture_output=True, text=True,shell=True)
...
# Update authorized_keys on each VM with public keys from other nodes
    for vm_name, public_key in ssh_keys.items():
        ...
        for other_vm, other_key in ssh_keys.items():
            if vm_name != other_vm:
                subprocess.run(f"multipass exec {vm_name} -- bash -c \"echo'{other_key}' >> /home/ubuntu/.ssh/authorized_keys\"", shell=True)
                subprocess.run(f"multipass exec {other_vm} -- ssh {vm_name} -q -o StrictHostKeyChecking=no echo '{other_vm}'", shell=True)
...
def setup_nfs_common(size):
    for i in range(1, size):
        ...
        fstab_content = "manager:/home/ubuntu/cloud /home/ubuntu/cloudnfs"
...
def create_mpi_hosts(size):
    ...
    for i in range(size):
        vm_name = f"manager" if i == 0 else f"worker{i}"
        subprocess.run(f"multipass exec manager -- bash -c \"echo'{vm_name}' >> /home/ubuntu/cloud/mpi_hosts\"", shell=True)
